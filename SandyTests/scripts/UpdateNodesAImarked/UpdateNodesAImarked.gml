...
start = argument0;
var char = argument1, ignore = argument2;
			...
			if(ds_priority_find_priority(open, neighbor) == 0 || ds_priority_find_priority(open, neighbor) == undefined)
			{
				...
				if(mir == 1 && mar == 1)
				{
					for(var a = 0; a < ds_list_size(neighbor.neighbors); a ++)
					{
						...
						if(abs(neighbor.level - tile.level) < 2 && tile.marked < 1000)
						{
							if(tile != start)tile.marked += 1000;
						}
					}
				}
...
with(obj_node)
{
	if((start != id && global.tileBinary[gridY] & 1<<gridX) )
	{
		marked ++;
	}
	else if(marked >= 1000)
